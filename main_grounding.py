...
def train_epoch(epoch, args, model, train_dataloader, device, n_gpu, optimizer,
    ...
    for step, batch in enumerate(train_dataloader, start=1):
        ...
        text_ids, text_mask, video, video_mask, inds, idx, moment = batch
...
def _run_on_single_gpu(model, t_mask_list, v_mask_list, t_feat_list, v_feat_list, cls_list, mini_batch=32, gauss=None):
    sim_matrix = []
    selected_props = []
    ...
    with torch.no_grad():
        for _, (t_mask, t_feat, cls, v_mask, v_feat) in enumerate(zip(batch_t_mask, batch_t_feat, batch_cls_feat, batch_v_mask, batch_v_feat)):
            ...
            if gauss:
                ...
                t_mask = t_mask[:, : -1]
                v_mask = v_mask[:, : -1]
            loss1, loss2, _, _, _, _, _, idx =model.get_moment_text_rec(t_feat, v_feat, t_mask, v_mask, props, text_weight)
...
def eval_epoch(args, model, test_dataloader, device):
    ...
    # ----------------------------
    batch_mask_t, batch_mask_v, batch_feat_t, batch_feat_v, ids_t, ids_v, batch_moment = [], [], [], [], [], [], []
    batch_cls = []
    ...
    with torch.no_grad():
        ...
        if multi_sentence_:  # multi-sentences retrieval means: one clip has two or more descriptions.
            ...
            for batch in tqdm(test_dataloader):
                batch = tuple(t.to(device) for t in batch)
                text_ids, text_mask, video, video_mask, inds, _, moment = batch
            ...
            batch_moment = allgather(torch.cat(batch_moment, dim=0), args)
            ...
            batch_moment[ids_t] = batch_moment.clone()
            ...
            batch_moment = batch_moment[:ids_t.max() + 1, ...]
            logger.info('[finish] extract text+video feature')
    ...
    with torch.no_grad():
        selected_props = _run_on_single_gpu(model, batch_mask_t, batch_mask_v, batch_feat_t, batch_feat_v,
                                                      batch_cls, gauss=args.do_gauss)
        selected_props = np.concatenate(tuple(selected_props), axis=0)
