...
class ActivityNetDataset_Grounding(Dataset):
    def __init__(
            self,
            subset,
            data_path,
        ...
        video_json_path_dict["val"] = os.path.join(self.data_path, "val_2.json")
    ...
    def _get_captions_single(self, path):
        ...
        for pseudo_video_id, v_ in json_data.items():
            ...
            for timestamp, sentence in zip(v_["timestamps"], v_["sentences"]):
        ...
        return pseudo_caption_dict, annotations

    def _get_text(self, pseudo_video_id, sub_id, anno):
        caption = self.pseudo_caption_dict[pseudo_video_id]
        k = 1
        r_ind = [sub_id]
        ...
        for i in range(k):
            ...
            words = self.tokenizer.tokenize(anno['sentence'])
    ...
    def __getitem__(self, feature_idx):
        ...
        video, video_mask = self._get_rawvideo(ann['vid'], starts, ends)
        return pairs_text, pairs_mask, video, video_mask, feature_idx, hash(ann['vid']), ann['moment']
